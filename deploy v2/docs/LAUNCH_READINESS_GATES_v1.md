...
>
> **⚠️ RECONCILED 2026-06-10 (live = b13/v182, MoJ 161d) — read this register WITH these corrections (authoritative forward state = `RISK_SUMMARY.md` + Session_Log §20.45–§20.47):**
> - **Beta is NOT a launch gate.** Per **ISS-G03**, the «gate #6 = beta cohort» of the #65a/governance numbering (distinct from *this* register's gate 6 = Condition model) is **DELETED**; the beta is a **parallel, non-blocking GT-collection track** — collect GT manually + organically, **no cohort, no cohort gate**. The conservative **R13 cover** (FREE / invite-only / «تقدير سوقي آلي، وليس تقييماً معتمداً» / no paid access) is **PRESERVED** — a *cover*, not a beta ritual.
> - **Gate 4 (accuracy) reframed:** expert **spot-check on the manual GT now**; a statistical error **distribution** is a **public-tier** milestone, not a beta blocker. Income work is **UX-gated**, not beta-gated.
> - **Stale gate statuses corrected inline below:** gate 6 over-anchor half **SHIPPED** (b6 → b11 cost down-re-anchor → b13 convergent-TRIM); the under-anchor half is **B-2** PARKED n≥20 (E25). gate 9 = **A5 only** (A7 ✅ CLOSED a20 · A15 ✅ CLOSED b12). gate 11 surface already hardened by a16.
...
| 3 | **PDPPL compliance** | L | **SELF-CLEARED conservative 2026-06-02 for the FREE track** (COMPLIANCE_SELF_CLEARANCE_beta_v1 + DPIA + a24 consent/log-scrub); capture **DORMANT** pending D-2 (§8.1/§8.2) + gate 11; counsel at PAID/PUBLIC | ⚠️ partly (you initiate; regulator's clock) | PUBLIC, B2B (likely BETA too) | Public handling of citizens' transaction data almost certainly requires it. **Possible long pole.** Needs legal counsel, not a sprint. |
...
| 5 | **Public ToS / liability / disclaimer** | L | **SHIPPED for beta** (a24: Terms modal + consent gate + «ليس تقييماً معتمداً» live); formal pass at PUBLIC | ✅ (with counsel) | PUBLIC, B2B | "Not a certified valuation" framing + limitation of liability. Light at BETA, formal at PUBLIC. |
| 6 | **Condition model (Branch B)** — over-anchor half ✅ SHIPPED | S | **B-2 under-anchor half PARKED (n≥20)** | ✅ | (accuracy-relevant) | The R7 **over-anchor** half SHIPPED: b6 widen_down → b11 cost down-re-anchor → b13 convergent-TRIM (Marikh; D-1 0.31 floor ✅). The **under-anchor** half = **B-2** — a calibrated `luxury_new` comparable stratum (**E25:** premium-above-cost, not cost-reachable) — PARKED on GT **n≥20**. Engine input for 2.22.0b Stage 2. |
| 7 | **5-stage UX (2.22.0b)** | S | **SUBSTANTIALLY SHIPPED** (b2.1/b2.2/b2.3/b3/b9/b10…b13 — screens 1-3 + range-as-lead live); remaining = screens 4-5 (polished result + report), unblocked by b14; the old "gated on B" is **void** | ✅ | PUBLIC (the consumer value prop) | A sprint *cluster*, not one. Stages 4/5 (broker/valuer) depend on gate 8. |
...
| 9 | **Open mediums — A5 only** (~~A7~~ ✅ CLOSED a20 · ~~A15~~ ✅ CLOSED b12) | S | A5 open | ✅ | PUBLIC | A7 (`rics_compliant` honest-status label) shipped a20/v159; A15 (HBU-not-evaluated disclosure) shipped b12/v181. Remaining open Medium = **A5** (`asset_type unknown`), small. |
