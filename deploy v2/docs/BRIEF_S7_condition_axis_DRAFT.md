...
This is the crux. A proposed mapping (PO to sign / amend):

| user attestation | leads with | guard |
|---|---|---|
| تحتاج ترميماً / teardown | **cost floor / land-priced stratum** (unchanged) | never raised |
| متوسّطة + عاديّ | **cost floor** (conservative — no lift without a positive signal) | never raised |
| **جيّدة + عاديّ** (not old) | **modern stratum** (1.5–2.2× land, n≥10) | only if that stratum is reliable (n≥10) AND ≥ cost floor |
| **جيّدة/ممتازة + راقٍ** | **luxury stratum** (≥2.2× land, n≥10) | only if reliable AND the finish attestation is «راقٍ» |

**Open calibration question (honesty):** does «good condition, 17yr, ordinary finish» really == the modern stratum? The r9 verdict is «indicative at n=11» — **disclosed, NOT calibrated to n≥20**. The durable calibration (the b71 `condition_adjustments.sqlite`, currently n=1 seed from V001) tightens only with **documented GT (n≥20)** — the PO's «الرقم يتغيّر لا الكود» decision. So S7 ships the mapping as **indicative + disclosed**, and it self-tightens as GT arrives (no code change — «الرقم يتغيّر لا الكود»).
