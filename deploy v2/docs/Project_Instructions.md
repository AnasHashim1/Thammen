...
> **آخر تحديث:** 2026-05-22 (الإصدار 7) — بعد Sprint 2.21.0.7.1 (Asset Type Reality Check). يدمج كل ما سبق (ماراثون 2026-05-17/18 + المثمن §20.8 + Bug A11 + Pydantic A2) **بالإضافة إلى** قوس الأراضي (Land Arc): 2.19 Cap-Rate Calibration · 2.20 Land Comparable Grid · 2.21.0 PIN Input · 2.21.0.5 Output Polish · **2.21.0.7/.7.1 Asset Type Reality Check** (QARS-in-polygon + General_Landuse RULEID؛ built→stop/reject، bare→value/reject حسب RULEID الموثّق). Engine الحالي: `thammen-sprint2p21p0p7p1-hotfix-removed` (v91). قواعد جديدة: Operational #45-#49، Empirical E8-E14.
...
|---|---|---|---|
|**1**|**2.18.1**|**Parallel BFS in `_expand_extent` (kill HTTP 503 class on compound_small)**. Audit-driven (Rule #51). Target: 51/835/17 from ~89s → ~5-8s. Approach: `ThreadPoolExecutor` over neighbour-PIN fetching in `_expand_extent` (qatar_gis.py); deterministic merge of boundary-sharing results; bounded `as_completed` consumption. Effort: 2 days. Same §5 audit-driven pattern as Sprint 2.18.0 — pre-Sprint audit on compound_small cases + CHANGELOG with predicted timings + post-deploy comparison. **Status: ready to start on Anas approval.**|none (data + tooling in place from Sprint 2.18.0)|
|**2**|**2.16.16**|**Confirmed Sales DB integration** (renumbered 2.16.13 → 2.16.15 → 2.16.16 — A11 and A2 took intermediate slots)|بيانات السكرتيرة (الخميس 2026-05-21)|
|3|2.17|QARS local snapshot|priorities post-Thursday|
|4|2.18.2 candidate|Lite/full GIS deduplication — closes villa/land Stage-1 (≤5s) gap (~22s→~12s). Deferred until 2.18.1 ships.|2.18.1 first|
|5|~~2.18~~|~~A6 latency + async landmarks + BUA-aware sanity~~ — **SUPERSEDED** by Sprint 2.18.0 (deployed 2026-05-23, CHANGELOG_v44) + Sprint 2.18.1 (queued). Original 2.18 was a bundle; split into single-purpose Sprints per Rule #38.|—|
|6|2.20|A8 Comparable adjustments grid (full size + corner). Time-only grid shipped Sprint 2.20.0; size deferred (R²≈0.05); corner BLOCKED by E12.|design + confirmed sales|
|7|2.29|MME apartments integration|MME auth flow|
...
|ID|Bug|Target|
|---|---|---|
|A6|Latency. **Phase 1 deployed 2026-05-23 (Sprint 2.18.0, v44)**: villa/raw_land paths cut −4s via parallel property_factors (multi_qars_56 26.8s→22.8s; khor_land 25.1s→21.2s — measured). **Phase 2 (Sprint 2.18.1) queued**: parallel BFS in `_expand_extent` to kill the HTTP 503 class on compound_small (51/835/17 still ~89s/HTTP 503).|2.18.1|
