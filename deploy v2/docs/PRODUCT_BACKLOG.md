...
>
> **الحيّ الآن:** b42 / Heroku **v213** / `master==origin @45a9701` (2026-06-14).
...
| ~~**+م**~~ | **نسخة-المُشغِّل بالبريد** (ذاكرة كلّ تقرير + نسخة تصل المُشغِّل) — كل تقييم → بريد Resend؛ صندوق المُشغِّل = الذاكرة (لا DB، FS عابر)؛ خامل افتراضياً (`RESEND_API_KEY`+`REPORT_COPY_EMAIL`) | 🟢 backend / dormant / value-invariant | ✅ **SHIPPED b42 / Heroku v213** (`45a9701`، §20.73/CHANGELOG_v125) — **⏳ المتبقّي:** اختبار الإرسال الحيّ بعد مفتاح Resend؛ توسيع البيتا يحتاج تحديث إشعار a24 «لا نخزّن العنوان» (Rule #39). يكمّل البند #6 (a15 D-2 = الالتقاط الكمّي المُجمَّع، محجوب بمشورة) | §20.73 |
