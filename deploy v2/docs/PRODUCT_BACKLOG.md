...
>
> **الحيّ الآن:** b42.1 / Heroku **v215** / `master==origin @0efec63` (2026-06-14).
...
| ~~**+م**~~ | **نسخة-المُشغِّل بالبريد** (ذاكرة كلّ تقرير + نسخة تصل المُشغِّل) — كل تقييم → بريد Resend؛ صندوق المُشغِّل = الذاكرة (لا DB، FS عابر)؛ خامل افتراضياً (`RESEND_API_KEY`+`REPORT_COPY_EMAIL`) | 🟢 backend / value-invariant (الآن LIVE + ACTIVE) | ✅ **SHIPPED + LIVE-TESTED b42.1 / Heroku v215** (`0efec63`، §20.73/CHANGELOG_v125) — **✅ اختبار الإرسال الحيّ نجح** (مُفعَّل: `RESEND_API_KEY`+`REPORT_COPY_EMAIL` على Heroku؛ إصلاح b42.1 لحجب Cloudflare 1010 على `api.resend.com` بـUA متصفّح، #61)؛ **توسيع البيتا** يحتاج تحديث إشعار a24 «لا نخزّن العنوان» (Rule #39). يكمّل البند #6 (a15 D-2 = الالتقاط الكمّي المُجمَّع، محجوب بمشورة) | §20.73 |
