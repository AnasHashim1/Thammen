...
>
> **الحيّ الآن:** b42.2 / Heroku **v216** / `master==origin @0521871` (2026-06-14).
...
| ~~**+م**~~ | **نسخة-المُشغِّل بالبريد** (ذاكرة كلّ تقرير + نسخة تصل المُشغِّل) — كل تقييم → بريد Resend؛ صندوق المُشغِّل = الذاكرة (لا DB، FS عابر)؛ خامل افتراضياً (`RESEND_API_KEY`+`REPORT_COPY_EMAIL`) | 🟢 backend / value-invariant (الآن LIVE + ACTIVE + inbox-confirmed) | ✅ **SHIPPED + LIVE-TESTED b42.2 / Heroku v216** (`0521871`، §20.73/CHANGELOG_v125) — **✅ اختبار الإرسال الحيّ نجح + أكّده Anas في صندوقه** (مُفعَّل: `RESEND_API_KEY`+`REPORT_COPY_EMAIL`؛ إصلاح b42.1 لحجب Cloudflare 1010 على `api.resend.com` بـUA متصفّح #61؛ إصلاح b42.2 تنظيف حقلَي الرسالة — العمر dict→«≥N سنة» + التسمية العربيّة، الـfixture طابق الإنتاج E14)؛ **توسيع البيتا** يحتاج تحديث إشعار a24 «لا نخزّن العنوان» (Rule #39). يكمّل البند #6 (a15 D-2 = الالتقاط الكمّي المُجمَّع، محجوب بمشورة) | §20.73 |
