...
- **The two threading fixes (there are TWO material_uncertainty assembly paths):**
  - **Fast / refusal path** (`_enrich_material_uncertainty`): the regime_muc merge changed from `if v is not None and k not in out` to `if v is not None and out.get(k) is None` — a caller's `None` slot was shadowing regime_muc's value; filling `None`/absent slots is the correct, strictly-additive merge semantics. (Confirmed live: 52/903/90 refusal → `muc_basis_en` now present.)
  - **Valued main path** (`assess_uncertainty` → `UncertaintyLevel` → `evaluate_v3`): the dataclass carried only the *old 4* muc fields, so the valued villa's `regime_muc()` result (built at material_uncertainty.py) copied `muc_clause_ar/en + muc_basis_ar + muc_review_recommendation_ar` but **not** the new `_en` twins. `muc_review_recommendation_en` still surfaced via the `en_localize` catalog (static string); `muc_basis_en` could not (dynamic date/days → catalog-miss). Fix: added `muc_basis_en` + `muc_review_recommendation_en` to the `UncertaintyLevel` dataclass + the regime_muc copy (both early-return branches shape-consistent) + the `evaluate_v3` main-path MU dict. Additive, `_ar` untouched → value-invariant.
