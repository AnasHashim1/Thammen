...
> - **Live:** Heroku **v158** · engine `thammen-sprint2p22p0a19-thin-path-condition-caveat` (a19) · capture **DORMANT** · `master == origin` (sprint tip `ca220df` + this docs-close; read live via #57).
...
> - **Engineering = ACTIVE on signed briefs (#32)** — last shipped **a19** (thin-path condition caveat — COPY-ONLY/honesty-additive, **NO valuation logic, every value byte-identical**; extends the a17 villa/house condition-not-assessed caveat **path-complete** to thin + non-dispersed widened + preliminary via a one-constant `_CONDITION_NOTE_METHODS` broadening, keeping the `gate.get('gated')` exclusion so dispersed bracket/widened keep their a14/a10 honest-range [note never duplicates]; a18 had put Marikh 54/541/6 on the thin path ~5.4M → it now discloses condition; `api.py`+`index.html` UNTOUCHED [R14]; Heroku v158, CHANGELOG_v71, §20.19; isolated a19 22/22 + a17 15/15 + DoD 392/15/45/62; live smoke v158: 54/541/6 = comparison_thin 5.4M + note PRESENT, 56/565/21 = comparison_bracket 2.4M + note PRESENT, 52/903/90 refusal; +`.gitignore` git-hygiene [untracked ~180→15, all scratch]; Pearl override جزيرة اللؤلؤة→اللؤلؤة confirmed KEPT/can't-mix-precincts). **Engineering NEXT = A7** (`rics_compliant` surfacing — beta-credibility quick-win, carries a copy sign-off) then **Sprint B** (durable R7 built-type/condition axis). Prior: a18 (R9 area-name reconciliation, sibling aggregation, v157, §20.18). Monetization still gated on the regulatory enquiry + open-data licence.
